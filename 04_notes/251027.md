
# 🧩 ROS2 주문 수신 노드 구현 및 실행 가이드

## 0. 디렉터리 구조
```
ros2_ws/
└── src/
    └── scout_robot/
        ├── package.xml
        ├── setup.py
        ├── setup.cfg
        ├── scout_robot/
        │   ├── __init__.py
        │   └── assign_order_node.py

```

## 1. ROS2 패키지 생성
터미널 열고 작업 폴더로 이동:
```bash
cd ~/ros2_ws/src
```

패키지 생성 (Python 기반):
```
ros2 pkg create --build-type ament_python scout_robot
```

## 2. 코드 추가
패키지 폴더로 이동:
```bash
cd ~/ros2_ws/src/scout_robot/scout_robot

# assign_order_node.py 파일 생성
```
패키지 코드 작성:
```python
import rclpy
from rclpy.node import Node
from std_msgs.msg import String

class ScoutAssignOrderNode(Node):
    def __init__(self):
        super().__init__('scout_assign_order_node')
        # /scout/assign_order 토픽 구독
        self.subscription = self.create_subscription(
            String,
            '/scout/assign_order',
            self.order_callback,
            10
        )

    def order_callback(self, msg: String):
        order_number = msg.data
        self.get_logger().info(f'배달 주문 수신: {order_number}')
        self.start_delivery(order_number)

    def start_delivery(self, order_number: str):
        self.get_logger().info(f'주문 {order_number} 배달 시작!')

def main(args=None):
    rclpy.init(args=args)
    node = ScoutAssignOrderNode()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()

```

## 3. `setup.py` 수정
```
entry_points={
    'console_scripts': [
        'assign_order_node = scout_robot.assign_order_node:main',
    ],
},

```

## 4. `package.xml` 확인
의존성 추가:
```
<exec_depend>rclpy</exec_depend>
<exec_depend>std_msgs</exec_depend>
```


## 5. 빌드
워크스페이스 루트로 이동 후 빌드:
```
cd ~/ros2_ws
colcon build --packages-select scout_robot
```
빌드가 완료되면 환경설정 적용:
```
source install/setup.bash
```

## 6. 노드 실행
```
ros2 run scout_robot assign_order_node
```
터미널에 이런 로그가 보이면 성공 👇
```
[INFO] [1698390000.000000000] [scout_assign_order_node]: 배달 주문 수신: ...

```

## 7. 테스트 - 토픽 발행
다른 터미널을 열고 (환경변수 다시 적용):
```
source ~/ros2_ws/install/setup.bash
ros2 topic pub /scout/assign_order std_msgs/msg/String "data: 'ORDER123'"



# nav 패키지 명령어
ros2 topic pub --once /room_command std_msgs/String "data: 'go_room1'"

# 배달 시작 음성 명령어
ros2 topic pub --once /speaker_command std_msgs/String "data: 'start_delivery'"
# room 정보 함께 전달
ros2 topic pub --once /speaker_command std_msgs/String "data: '{\"command\": \"start_delivery\", \"room\": \"101\"}'"


# 명령어 합치기
ros2 topic pub --once /speaker_command std_msgs/String "data: 'start_delivery'"& \
ros2 topic pub --once /room_command std_msgs/String "data: 'go_room1'" 


```

#### 참고

```
# 패키지 확인 명령어
ros2 pkg executables scout_robot
```

