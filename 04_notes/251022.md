...
```
# ros-bridge(ssh)
ros2 launch rosbridge_server rosbridge_websocket_launch.xml compression:=none

# 스피커 노드(ssh)
ros2 run speaker_pkg speaker_node

# 초음파 노드(ssh)
ros2 run ultrasonic_delivery_sensor ultrasonic_node

# uart 노드(ssh)
ros2 run uart_bridge uart_sender_node

# 웹캠 노드(ssh)
ros2 run v4l2_camera v4l2_camera_node \ --ros-args \ -p video_device:="/dev/video0" \ -p image_size:="[640,480]" \ -p pixel_format:="YUYV" \ -p frame_rate:=5 \ -p image_transport:=compressed

# nav2 관련(local)
ros2 run scout_robot qr_detector
ros2 run scout_robot nav2_commander
ros2 run scout_robot amcl_reset
ros2 run scout_robot robot_rotator
...
```
scp eddy@192.168.123.250:/home/eddy/ros2_ws/maps/slamdunk.pgm ~/ros2_ws/maps/
```

- 라즈베리파이(ssh)로 파일 업로드
```
scp *.wav eddy@192.168.123.250:/home/eddy/sound/
```
